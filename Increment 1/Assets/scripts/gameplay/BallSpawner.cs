...
public class BallSpawner : MonoBehaviour
{
    ...
    public void SpawnBall()
    {
        ...
        {
            // Creates a random selection of which ball to spawn based on balls percentage
            float choose = Random.Range(0.0f, 1.0f);
            retrySpawn = false;
            if (choose < ConfigurationUtils.StandardBallSpawnPercentage)
            {
                Instantiate(prefabBall, Vector3.zero, Quaternion.identity);
            }
            else if (choose < ConfigurationUtils.BonusBallSpawnPercentage + 0.5)
            {
